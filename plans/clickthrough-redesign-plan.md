...
- **C2 プローブ実施済み**: through 状態（`set_ignore_cursor_events(true)`）でメモカードをクリックしても `pointerdown` が JS に届かないことを実機で確認。
- **技術的根拠**: macOS の `NSWindow.ignoresMouseEvents = true` は OS がウィンドウへのイベント配送そのものを止める。CSS の `pointer-events` はその後段にしか効かない。
- **代替技術の評価**: ObjC `hitTest` オーバーライドによる領域別透過は原理的には可能だが、メモカード座標を Rust 側に常時同期するコストが高く、採用しないと判断した。
- **採用した最終形**: 手動トグル（overlay ↔ through）+ tray アイコン状態表示。自動復帰（C1b）付き。
- **比較案（feature_window_model_redesign）の扱い**: 削除せず比較結果として保管する。「なぜ click-through 本命案を捨てたか」「memo 1 window 案と何が違うか」の参照点として機能させる。

本計画書はこれ以上更新しない。経緯の参照文書として保管する。
...
- 2026-04-11: C4 NO 確定。実験結果セクション（§14）を追記。本計画書を経緯参照文書として凍結。
